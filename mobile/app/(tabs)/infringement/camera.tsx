...
export default function CameraScreen() {
  ...
  const [tempPhotoForWatermark, setTempPhotoForWatermark] = useState<
    string | null
  >(null);
  ...
  const takePicture = async () => {
    ...
    try {
      ...
      Alert.alert(
        "Photo Captured",
        `Photo watermarked successfully! (${Math.round(
          compressedPhoto.size / 1024
        )}KB). ${MAX_PHOTOS - photos.length - 1} remaining.`
      );
    } catch (error) {
  ...
  const pickImage = async () => {
    try {
      ...
      if (!result.canceled && result.assets[0]) {
        ...
        Alert.alert(
          "Photo Added",
          `Photo added from gallery (${Math.round(
            compressedPhoto.size / 1024
          )}KB)`
        );
      }
  ...
  const uploadAllPhotos = async () => {
    ...
    try {
      ...
      Alert.alert("Upload Failed", "Failed to upload photos. Save offline?", [
        { text: "Cancel", style: "cancel" },
        {
          text: "Save Offline",
          onPress: async () => {
