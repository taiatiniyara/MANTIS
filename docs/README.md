# MANTIS Documentation

Welcome to the **MANTIS** (Multi-Agency National Traffic Infringement System) documentation.

MANTIS is a unified, GIS-enabled enforcement platform built for the Fiji Islands, enabling LTA, Municipal Councils, and Fiji Police to manage traffic infringements through both a web application and mobile app.

## 📚 Documentation Structure

The documentation is organized into focused sections for easy navigation:

### 📱 Mobile App
Mobile app documentation for iOS and Android (Expo + React Native)
- [App Overview](./mobile/app-overview.md) - Features, tech stack, and architecture
- [Quick Start](./mobile/quickstart.md) - Get the mobile app running
- [Testing Guide](./mobile/testing.md) - Test scenarios and procedures
- [Theme Implementation](./mobile/theme-implementation.md) - UI components and theming
- [Refactoring Guide](./mobile/refactoring.md) - Code optimization patterns

### 🌐 Web Application
Web app documentation for the browser-based PWA (React + Vite)
- [Web App Guide](./web/README.md) - PWA features, setup, and architecture

### 🏗️ Architecture
System design and technical architecture
- [System Architecture](./architecture/system-architecture.md) - Overall system design
- [Tech Stack](./architecture/tech-stack.md) - Technologies and frameworks
- [Data Model](./architecture/data-model.md) - Database schema and relationships
- [GIS & Locations](./architecture/gis-locations.md) - Geographic information system
- [Authentication](./architecture/authentication.md) - Auth and authorization

### 🛠️ Development
Developer resources and reference materials
- [Getting Started](./development/getting-started.md) - Installation and setup
- [Deployment](./development/deployment.md) - Production deployment guide
- [Environment Variables](./development/environment-variables.md) - Configuration reference
- [Troubleshooting](./development/troubleshooting.md) - Common issues and solutions
- [FAQ](./development/faq.md) - Frequently asked questions
- [Glossary](./development/glossary.md) - Terms and definitions

## 🚀 Quick Start

Choose your platform to get started:

- **Mobile App**: See [mobile/quickstart.md](./mobile/quickstart.md)
- **Web App**: See [development/getting-started.md](./development/getting-started.md)
- **Backend Setup**: See [development/getting-started.md](./development/getting-started.md)

## 🔗 Additional Resources

- [Contributing Guidelines](./CONTRIBUTING.md) - How to contribute to MANTIS
- [Project Overview](./overview.md) - High-level platform guide
- Shared Supabase backend for both platforms

## 📞 Support

For questions or issues:
- Review the [troubleshooting guide](./development/troubleshooting.md)
- Check the [FAQ](./development/faq.md)
- Open an issue on GitHub
- Contact the development team

---

**Version:** 2.0.0  
**Last Updated:** January 16, 2026
